...
Создай `.claude/.cache/state.json`. Если Write отклонён — верни error (без state.json bootstrap невозможен).
...
Нет .claude/                                → fresh
Есть .claude/ но пуста (0 .md в agents/skills/pipelines/) → fresh
