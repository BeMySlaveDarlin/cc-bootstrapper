...
Проверь что каждый `.claude/pipelines/*.md` содержит YAML frontmatter с полями `name`, `phases` (array), `modes`. HTML-комментарий `<!-- version -->` → `[WARN]`.
...
В fresh/upgrade mode (registries заполнены):
...
В patch mode (registries пустые):
1. Прочитать `known-templates.json` (путь из prompt оркестратора)
2. Собрать множество template-путей по known-templates (active entries, подставить langs из state.stack.langs)

Общее для всех mode:
- `scripts/verify-bootstrap.sh` — всегда template
- `scripts/hooks/*.sh` — не включать в manifest (пользовательские, protected)
- Для каждого файла: если путь в множестве template-путей → `"source": "template"`, иначе → `"source": "user"`
