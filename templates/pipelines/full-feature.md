...
Agent(name="analyst", team_name=T)
  → SendMessage(to="lead", summary="done + spec path", message="done + spec path")
Gate: review
Cleanup: shutdown analyst

# Phase 2: ARCHITECTURE
Agent(name="{lang}-architect", team_name=T)
  → SendMessage(to="lead", summary="done + plan path", message="done + plan path")
Gate: review
...
Agent(name="{lang}-reviewer", team_name=T)
  {lang}-developer → SendMessage(to="{lang}-test-developer", summary="done + impl path", message="done + impl path")
  {lang}-test-developer → SendMessage(to="{lang}-reviewer", summary="done + tests path", message="done + tests path")
  {lang}-reviewer: BLOCK → SendMessage(to="{lang}-developer", summary="fix + comments", message="fix + comments")
                   PASS → SendMessage(to="lead", summary="verdict + review path", message="verdict + review path")
Gate: confirm
...
# Lead направляет developer на Phase 7:
  Lead → SendMessage(to="{lang}-developer", summary=""Phase 7: API_DOCS. Сгенерируй контракты."", message=""Phase 7: API_DOCS. Сгенерируй контракты."")
  {lang}-developer → SendMessage(to="lead", summary="done + contracts path", message="done + contracts path")
Gate: silent

# Phase 8: QA_DOCS (если has_api_endpoints)
# Lead спавнит qa-engineer после Phase 7 gate:
Agent(name="qa-engineer", team_name=T)
  → SendMessage(to="lead", summary="done + checklist path", message="done + checklist path")
Gate: silent
