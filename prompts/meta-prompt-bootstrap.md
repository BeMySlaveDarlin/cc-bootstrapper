...
```
VERSION_CURRENT = "3.0.0"
```

## CHANGELOG

### 3.0.0 (2026-02-13)
- Опциональная интеграция GitLab MCP (@zereight/mcp-gitlab)
- Генерация .mcp.json, агента gitlab-manager, скилла gitlab, пайплайна gitlab
- AskUserQuestion для сбора параметров подключения (URL, username, token)
...
HASHES="{}"
for f in .claude/agents/*.md .claude/skills/*/SKILL.md .claude/pipelines/*.md .claude/scripts/hooks/*.sh .claude/scripts/verify-bootstrap.sh .mcp.json; do
    [ -f "$f" ] || continue
...
jq -n \
    ...
    '{
        ...
        generated: {agents: $agents, skills: $skills, hooks: $hooks, mcp: (if env.GITLAB_MCP == "true" then ["gitlab"] else [] end)},
        hashes: $hashes
