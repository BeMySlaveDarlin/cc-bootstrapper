...
Автоопределение: нет `.claude/` → полная генерация, есть → валидация + миграция, есть state → resume.
...
## Процесс bootstrap (9 шагов)

| Шаг | Название | Что делает |
|-----|----------|------------|
| 1 | Сканирование | Light scan: манифесты, структура, стек |
| 2 | Определение режима | fresh / validate / resume |
...
| 8 | Генерация | Per-domain параллельно: per-lang + common + infra + CLAUDE.md |
| 9 | Финализация | Верификация, .bootstrap-version, cleanup |
...
|----------|-------|---------------|
| `new-code` | Новый модуль, сервис, эндпоинт | Analysis → Architecture → Storage → Code → Tests → Review |
| `fix-code` | Баг, ошибка, regression | Diagnosis → Fix → Tests → Review |
| `review` | Ревью кода | Per-lang Review (параллельно) → Report |
| `tests` | Написание тестов | Analyze → Generate (per-lang) → Verify → Review |
| `api-docs` | API-контракты | Scan → Generate → Save |
| `qa-docs` | Чеклисты, Postman, E2E | Input → Checklist → Automation (Postman + Playwright) → Save |
| `full-feature` | Полный цикл фичи | new-code + api-docs + qa-docs |
| `hotfix` | Срочное исправление | fix-code + review |

Мультиязычные проекты: developer, test-developer и reviewer запускаются **per-lang параллельно**.
...
| `storage/` | Хранилища: БД, кэш, очереди, object storage |
| `testing/` | Тест-фреймворк, моки, E2E |
