...
- Есть `.claude/` v8.x → patch (mini-upgrade) или upgrade
...
| 2 | Определение режима | empty / fresh / patch / upgrade / resume |
...
| 10 | Финализация | Верификация, .bootstrap-manifest.json, cleanup |
...
8 базовых + кастомные. v9 frontmatter: phases[] array, agents{}, modes, structured error_routing.
...
| `full-feature` | Полный цикл фичи | 8 inline phases: Analysis→...→QA | — |

4 пайплайна поддерживают **Agent Teams** (new-code, fix-code, tests, brainstorm): TeamCreate → Agent spawn → SendMessage координация → TeamDelete. Fallback на sequential автоматический.
...
В patch mode: diff-based merge с маркерами `[KEEP]`/`[+ADD]`/`[-DEL]`/`[USER]`.
